    ...
    RSpec.describe CreateMedication, type: :request do
      describe ".resolve" do 
        it "creates a medication" do 
          ...
          post '/graphql', params: { query: query(conditionId: condition.id, name: "Tylenol", datePrescribed: Date.today, dosage: "250mg", frequency: "2 every 6 hours as needed", prescribedBy: "Dr Pepperdine") }
          data = JSON.parse(response.body, symbolize_names: true)

          expect(Medication.count).to eq(1)
          expect(response).to be_successful 
          expect(data[:data]).to have_key(:createMedication)
          expect(data[:data][:createMedication].keys).to eq([:medication, :errors])
